...
public class ClusterProcessor<C> {
  ...
  public static class Builder<C> {
    ...
    public ClusterProcessor.Builder<C> withLifecycleFactory( Function<C, ClusterProcessLifecycle> lifecycleFactory ) {
    ...
    public ClusterProcessor<C> build() {
      ClusterProcessor<C> service = new ClusterProcessor<C>(directory, mapper, nodeId, type, lifecycleFactory);
  ...
  public ClusterProcessor(MappedEtcdDirectory<ClusterProcess> directory, ObjectMapper mapper, String nodeId, TypeReference<C> type, Function<C, ClusterProcessLifecycle> lifecycleFactory ) {
    this.directory = directory;
    this.mapper = mapper;
    this.nodeId = nodeId;
    this.type = type;
  ...
  protected void startProcess( String key, ClusterProcess node ) {
      ...
      if( existing != null ) safe(key, existing::stop);
