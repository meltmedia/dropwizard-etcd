...
public class WatchService {
  ...
  protected WatchService( Supplier<EtcdClient> client, String directory, ObjectMapper mapper, ScheduledExecutorService executor ) {
    this.client = client;
    this.directory = directory;
    this.mapper = mapper;
    this.executor = executor;
  ...
  public <T> Watch registerDirectoryWatch( String directory, TypeReference<T> type, EtcdEventHandler<T> handler ) {
    DirectoryWatch<T> watch = new DirectoryWatch<T>(this.directory+directory, type, handler);
    addWatch(watch);
    return watch;
  }
  
  public <T> Watch registerValueWatch( String directory, String key, TypeReference<T> type, EtcdEventHandler<T> handler ) {
  ...
  protected void startWatchingNodes() {
    ...
    watchFuture = executor.scheduleWithFixedDelay(( ) -> {
  ...
  static <T> Optional<EtcdEvent<T>> asEvent( ObjectMapper mapper, String directory, EtcdKeysResponse response, TypeReference<T> type ) {
    T value = readValue(mapper, response.node, type);
    T prevValue = readValue(mapper, response.prevNode, type);
    if( value == null && prevValue == null ) return Optional.empty();
    switch( response.action ) {
      case delete:
        return Optional.of(EtcdEvent.<T>builder()
          .withType(EtcdEvent.Type.removed)
          .withValue(value)
          .withPrevValue(prevValue)
  ...
  public class DirectoryWatch<T> implements Watch {
    ...
    public void init() {
      try {
        EtcdKeysResponse dirList = client.get().getDir(directory)
          .dir()
          .send()
          .get();
            ...
            .forEach(node->{
              ...
              if( value != null ) {
                fireEvent(handler, EtcdEvent.<T> builder()
                  ...
                  .withValue(value)
                  .withKey(removeDirectory(directory, node.key))
                  .build());
              }
              else {
    ...
    public void accept( EtcdKeysResponse response ) {
      ...
      if( currentIndex < responseIndex ) {
        asEvent(mapper, directory, response, type)
  ...
  public class ValueWatch<T> implements Watch {
    ...
    protected ValueWatch( String directory, String key, TypeReference<T> type, EtcdEventHandler<T> handler) {
      this.directory = directory;
      this.key = key;
      this.type = type;
      this.handler = handler;
    ...
    public void init() {
      try {
        EtcdKeysResponse response = client.get().get(directory+"/"+key)
        .send()
        .get();
  ...
  static String key( EtcdKeysResponse response ) {
    String key = response.node != null ? response.node.key :
      response.prevNode != null ? response.prevNode.key :
        null;
    
    if( key == null ) throw new IllegalStateException("no key defined.");
  ...
  static Long indexOf( EtcdKeysResponse response ) {
    return response.node != null ? response.node.modifiedIndex :
      response.prevNode != null ? response.prevNode.modifiedIndex :
        response.etcdIndex;
  }
